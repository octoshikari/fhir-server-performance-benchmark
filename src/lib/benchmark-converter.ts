...
function getTotalRPS(results: PrometheusResult[]): BenchmarkDataPoint[] {
  ...
  const dataPoint: BenchmarkDataPoint = {
    category: 'CRUD',
    aidbox: serverTotals.get('aidbox') || 0,
    medplum: serverTotals.get('medplum') || 0,
    hapi: serverTotals.get('hapi') || 0
  };
...
export function convertSourceToBenchmarkReport(
  ...
  // Convert CRUD data
  if (sourceData.crudTotalRPS && sourceData.crudP99) {
    const crudSuite: TypedBenchmarkSuite = {
      ...
      result: {
        ...
        unit: 'RPS',
        data: getTotalRPS(sourceData.crudTotalRPS.data.result)
      },
      test_cases: []
    };
    
    // Group P99 metrics by operation
    const groupedMetrics = groupMetricsByOperationAndResource(sourceData.crudP99.data.result);
  ...
  // Convert Search data (if available)
  if (sourceData.searchTotalRPS && sourceData.searchP99) {
    const searchSuite: TypedBenchmarkSuite = {
      ...
      result: {
        ...
        unit: 'RPS',
        data: getTotalRPS(sourceData.searchTotalRPS.data.result)
      },
      test_cases: []
    };
    
    // Group P99 metrics by operation
    const groupedMetrics = groupMetricsByOperationAndResource(sourceData.searchP99.data.result);
    
    // Create test cases for each operation
    for (const [operation, resourceGroups] of groupedMetrics) {
      const testCase: TypedTestCase = {
        label: `${operation.charAt(0).toUpperCase() + operation.slice(1)} search latency (Lower is better)`,
        description: `P99 in milliseconds per each resource type for ${operation} search operations`,
        unit: 'MS',
        data: createBenchmarkDataPoints(resourceGroups)
      };
